...
import { getCart as getGlobalCart, getCount as getCartCount, addItem as addCartItem, removeItem as removeCartItem } from '../ui/cart';
...
function StockPageInner() {
  ...
  // --- Cart state (global/localStorage) ---
  type CartRow = { id: string; title: string; thumb: string; price?: number };
  ...
  const addToCart = (item: DemoItem) => {
    addCartItem({ id: item.id, title: item.alt ?? item.id, thumb: item.src });
    setCartOpen(true);
  };
  ...
  // keep local view in sync with global cart
  useEffect(() => {
    const update = () => setCartItems(getGlobalCart());
    update();
                ...
                {cartItems.length > 0 ? (
                  <>
                    <div className="max-h-64 overflow-auto divide-y">
                      {cartItems.map((c) => (
                        <div key={c.id} className="flex items-center gap-2 py-2">
                          <img src={c.thumb} alt={c.title} className="h-10 w-14 object-cover rounded" />
                    ...
                    <div className="pt-2 flex items-center justify-between">
                      <div className="text-xs text-zinc-600">{cartItems.length} i kurv</div>
                  ...
                  <div className="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
                    {inCart(it.id) ? (
                      <button
                        ...
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); removeFromCart(it.id); }}
                      >
                        Fjern fra kurv
                      </button>
                    ) : (
                      <button
                        ...
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); addToCart(toDemo(it)); }}
                      >
                        Læg i kurv
                      </button>
                    )}
